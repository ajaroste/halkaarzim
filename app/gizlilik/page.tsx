...
export default function PrivacyPage() {
  return <LegalPage eyebrow="Gizlilik" title="Gizlilik ve KVKK Aydınlatma Metni" intro="Hangi verinin neden işlendiğini, hangi hizmetlerin kullanıldığını ve hesabın üzerindeki haklarını açık biçimde anlatır." version="1.1" toc={toc}>
    <section id="kapsam"><h2>1. Kapsam ve veri sorumlusu</h2><p>Bu metin HalkaArzım web sitesi, üyelik sistemi, yorumlar, takip listeleri, tarayıcı bildirimleri ve güvenlik kayıtları için hazırlanmıştır. Veri sorumlusunun yasal kimliği ve başvuru iletişim kanalı, hizmetin işletme yapısına uygun olarak bu sayfada ve ilgili iletişim alanlarında yayımlanır.</p><p>HalkaArzım bir aracı kurum, banka, portföy yönetim şirketi veya resmî kurum değildir.</p></section>
    ...
    <section id="paylasim"><h2>4. Hizmet sağlayıcılar ve aktarımlar</h2><p>Hizmetin çalışması için Supabase gibi kimlik doğrulama/veritabanı sağlayıcıları, Vercel gibi barındırma sağlayıcıları, e-posta teslim hizmetleri ve tarayıcı bildirim altyapıları kullanılabilir. Bu sağlayıcılar yalnız hizmetin gerektirdiği ölçüde veri işleyen konumunda olabilir.</p><p>Bu hizmetlerin altyapısı Türkiye dışında bulunabilir. Yurt dışı aktarım koşulları, geçerli mevzuat ve kullanılan sağlayıcıların sözleşmeleri çerçevesinde değerlendirilir; kişisel veri aktarımında yürürlükteki koruma mekanizmaları esas alınır.</p></section>

    <section id="ai"><h2>5. Yapay zekâ işlemleri</h2><p>AI sağlayıcılarına yalnız kamuya açık halka arz belgelerinden çıkarılan şirket verilerinin gönderilmesi hedeflenir. Kullanıcı e-postası, profil bilgisi, özel mesajı, takip listesi, push aboneliği veya yorum içeriği AI analizine gönderilmemelidir.</p><p>AI çıktıları otomatik olarak yatırım tavsiyesi hâline gelmez; kaynak ve ifade kontrollerinden geçirilir. Ayrıntılar <a href="/ai-politikasi">AI Kullanım Politikası</a> sayfasındadır.</p></section>

    <section id="saklama"><h2>6. Saklama, düzeltme ve silme</h2><ul><li>Hesap verileri hesap açık olduğu sürece; silme talebinden sonra hukuki veya güvenlik gereklilikleri saklı kalmak üzere kaldırılır ya da anonimleştirilir.</li><li>Yorum ve moderasyon kayıtları topluluk güvenliği, uyuşmazlık ve kötüye kullanım incelemesi için makul süreyle saklanabilir.</li><li>Push aboneliği kullanıcı bildirimi kapattığında veya abonelik geçersiz olduğunda silinir.</li><li>Güvenlik günlükları amaç için gerekli olandan uzun tutulmamalı; erişim yalnız yetkili kişilerle sınırlandırılmalıdır.</li></ul><p>Saklama süreleri işleme amacı, güvenlik ihtiyacı ve ilgili hukuki yükümlülüklerle sınırlı tutulur.</p></section>

    <section id="haklar"><h2>7. Kullanıcı hakları ve başvuru</h2><p>Kullanıcılar kendileriyle ilgili verilerin işlenip işlenmediğini öğrenme, bilgi talep etme, yanlış verileri düzeltme, şartları oluştuğunda silme veya anonimleştirme isteme ve hukuka aykırı işlem nedeniyle zarar doğduğunu düşünüyorsa başvuru yapma haklarına sahip olabilir.</p><div className="legalContactCard"><strong>Başvuru ve hesap işlemleri</strong><p>Hesap silme, içerik kaldırma ve güvenlik bildirimleri için sitedeki ilgili hesap/iletişim akışları kullanılabilir. Veri sorumlusu için resmî başvuru iletişim kanalı ilan edildiğinde bu bölüm aynı kanalı gösterecek şekilde güncellenir.</p></div></section>

    <section id="guvenlik"><h2>8. Güvenlik önlemleri</h2><p>Yetki kontrolleri, Supabase Row Level Security politikaları, şifreli HTTPS bağlantısı, sunucu tarafı gizli anahtar yönetimi, güvenlik başlıkları, hız sınırlaması, giriş doğrulama ve denetim kayıtları uygulanır. Buna rağmen internete açık hiçbir sistem için mutlak güvenlik garantisi verilemez.</p><p>Şüpheli hesap hareketi veya veri ihlali fark edildiğinde güvenlik bildirim kanalı kullanılmalı ve ilgili oturumlar sonlandırılmalıdır.</p></section>
  </LegalPage>;
}
