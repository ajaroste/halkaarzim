...
async function fetchKap(url: string, maxBytes: number, timeoutMs = 20_000) {
  ...
  if (parsed.protocol !== "https:" || !["kap.org.tr", "www.kap.org.tr"].includes(parsed.hostname)) {
    throw new Error("KAP host validation failed");
  }
  ...
  try {
    const response = await fetch(url, {
      redirect: "error",
      headers: {
        "user-agent": "HalkaArzim-KAP-Enrichment/1.0 (+https://halkaarzim.vercel.app)",
...
function isRecentEnough(payload: Record<string, unknown>, publishedAt: string | null) {
  ...
  return Date.now() - ts <= LOOKBACK_DAYS * 86_400_000;
}
...
function needsEnrichment(payload: Record<string, unknown>, ticker: string | null, publishedAt: string | null) {
  ...
  const hasTicker = Boolean(stringValue(payload.ticker) || ticker);
  const hasDates = Boolean(stringValue(payload.collectionStart) && stringValue(payload.collectionEnd));
  const hasFirstTrade = Boolean(stringValue(payload.firstTradeDate));
  const hasParticipant = Number(payload.participantCount || 0) > 0;
  return !hasTicker || !hasDates || !hasFirstTrade || !hasParticipant;
}
...
function mergeNotes(payload: Record<string, unknown>, note: string) {
  const existing = Array.isArray(payload.dataNotes)
    ? payload.dataNotes.filter((item): item is string => typeof item === "string" && Boolean(item.trim()))
    : [];
...
function pickCompany(value: unknown) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const row of targets) {
      ...
      try {
        const candidates = extractNotificationCandidates(indexHtml, companyName, MAX_NOTIFICATION_DETAILS_PER_IPO);
        if (!candidates.length) {
          results.push({ slug, company: companyName, candidates: 0, changed: false });
          continue;
        }
        matched += 1;
        ...
        for (const candidate of candidates) {
          ...
          if (parsed.collectionStart && parsed.collectionEnd && canReplaceSchedule) {
            ...
            if (parsedPublished) {
              enrichment.schedulePublishedAt = parsedPublished;
              newestScheduleDate = parsedPublished;
            }
          }
          kapSources.push({
            title: `KAP — ${companyName} halka arz bildirimi`,
            page: "Halka arz / satış takvimi bildirimi",
            kind: "KAP / resmî kayıt",
            url: candidate.url,
          });
        }
        ...
        if (!existingTicker && candidateTicker) {
          nextPayload.ticker = candidateTicker;
          changed = true;
        }
        ...
        for (const key of ["market", "firstTradeDate", "participantCount"] as const) {
          if (!nextPayload[key] && enrichment[key]) {
            nextPayload[key] = enrichment[key];
            changed = true;
          }
        }
        ...
        results.push({ slug, company: companyName, candidates: candidates.length, enrichment, changed, sources: kapSources.map((source) => source.url) });
      } catch (error) {
    ...
    if (runId) {
      await admin.from("ipo_sync_runs").update({
